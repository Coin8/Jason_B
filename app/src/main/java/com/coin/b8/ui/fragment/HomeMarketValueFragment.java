 ...
 */
public class HomeMarketValueFragment extends BaseFragment implements IHomeMarketNormalView,View.OnClickListener{
    ...
    private class MyHandler extends Handler{
        private WeakReference<HomeMarketValueFragment> mHomeMarketValueFragment;
        public MyHandler(HomeMarketValueFragment fragment) {
            mHomeMarketValueFragment = new WeakReference<HomeMarketValueFragment>(fragment);
        }

        @Override
        public void handleMessage(Message msg) {
            HomeMarketValueFragment fragment = mHomeMarketValueFragment.get();
            if(fragment == null){
                return;
            }
            switch (msg.what){
...
//        });
    ...
    private void startLoadMore(){
        int start = mHomeMarketValueAdapter.getItemCount() + 1;
        ...
        switch (type){
            ...
            case 1:{
                if(response != null
                        && response.getData() != null
                        && response.getData().size() > 0){
                    mHomeMarketValueAdapter.addList(response.getData());
                }
